...
$clstramiteusuario->setTu_id($idtue["tu_id"]);
...
/*ANIADIDO POR LOS ANEXOS
switch ($id_tramite){
    case "8": $clstramitee = new clstramite8();
        break;
    case "12": $clstramitee = new clstramite12();break;
}
echo "cod_tramite".$cod_tramite;
$clstramitee->setTu_codigo($cod_tramite);
$idtue=$clstramitee->tra_seleccionar_bycodigo();
$idtue= mysqli_fetch_array($idtue);*/
...
if($re1==0 || $re2==0){
    ...
    $cc=$tipot["tra_edirnac"];
    $namecc="Direcciones Nacionales INPC";
    ...
    $mensaje_especifico.="<p><b>Descargar respuesta:</b></p><p><a href='".URL_SIS.DIRDOWNLOAD.RUTA_ARCHIVOSTRAMITES.$cod_tramite."/".$cod_tramite.".pdf'>Respuesta</a></p>";
    $mensaje_especifico.="<p><b>Descargar anexos:</b></p>";
    //OBTENER ANEXOS
    $anexose=new clstuanexos();
//echo "t".$idtue["tu_id"]."i".$id_tramite;
    $anexose->setTu_id($idtue["tu_id"]);
    $anexose->setTra_id($id_tramite);
    $anexos=$anexose->tua_seleccionar_byte();
    if(mysqli_num_rows($anexos)>=1){
        while($anexo=mysqli_fetch_array($anexos)){  
            $mensaje_especifico.="<p><a href='".URL_SIS.DIRDOWNLOAD.$anexo["tua_rutaarchivo"]."'>".$anexo["anx_nombre"]."</a></p>";
        }
    }else{
	//echo "no tienen anexos";
        $mensaje_especifico.="<p>Sin anexos</p>";
    }
