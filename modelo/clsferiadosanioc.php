...
Class clsferiadosanioc{	 
    ...
    public function get_count_diasferiado(){
        $bd=Db::getInstance();
        $sql = "SELECT count(*) as total FROM ct_feriados_anio JOIN ct_feriados ON (ct_feriados.fer_id=ct_feriados_anio.fer_id) WHERE ('".$this->fa_fechafin."'>=fea_fecha and '".$this->fa_fechainicio."'<=fea_fecha) and (ct_feriados.fer_tipo='NACIONAL' or (ct_feriados.fer_tipo='LOCAL' AND ct_feriados.reg_id='".$this->fa_regional."'))";     
        //echo $sql;
